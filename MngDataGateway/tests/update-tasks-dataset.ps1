...
if (-not (Test-Path $TokenFile)) {
    Write-Host "   Token file not found: $TokenFile" -ForegroundColor $ErrorColor
    Write-Host "   Please run: cd C:\Serkan\iSIM\MonitraNG\MngKeeper\tests && .\get-serkan-token.ps1" -ForegroundColor Yellow
    exit 1
}

$token = Get-Content $TokenFile -Raw | ForEach-Object { $_.Trim() }
if ([string]::IsNullOrWhiteSpace($token)) {
    Write-Host "   Token is empty" -ForegroundColor $ErrorColor
    exit 1
}
