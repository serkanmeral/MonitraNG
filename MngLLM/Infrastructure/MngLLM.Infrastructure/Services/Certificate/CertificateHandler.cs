...
namespace MngLLM.Infrastructure.Services.Certificate;
...
public class CertificateHandler
{
    ...
    private static X509Certificate2 GetSignedCertificate(Serilog.ILogger log, MngLLMSettings settings)
    {
        X509Certificate2 certWithKey = new X509Certificate2();

        try
        {
            string crtVal = !string.IsNullOrEmpty(settings.CertificateSettings.CERT_FILE_CONTENT)
                ? settings.CertificateSettings.CERT_FILE_CONTENT
                : File.ReadAllText(settings.CertificateSettings.CERT_FILE);

            string keyVal = !string.IsNullOrEmpty(settings.CertificateSettings.KEY_FILE_CONTENT)
                ? settings.CertificateSettings.KEY_FILE_CONTENT
                : File.ReadAllText(settings.CertificateSettings.KEY_FILE);
    ...
    private static X509Certificate2 CreateSelfSignedCertificate(Serilog.ILogger log, string dns)
    {
        string countryName = "TR";
        string stateOrProvinceName = "ISTANBUL";
        string localityName = "UMRANIYE";
        string organizationName = "iSIM Platform";
    ...
    public static X509Certificate2 GetCertificate(Serilog.ILogger log, MngLLMSettings settings)
    {
