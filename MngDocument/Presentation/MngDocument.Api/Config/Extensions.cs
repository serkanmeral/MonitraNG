...
using MngDocument.Api.Filters;
using MngDocument.Application.Configuration;
using Microsoft.OpenApi.Models;
using Serilog;
using Swashbuckle.AspNetCore.SwaggerGen;

namespace MngDocument.Api.Config;
...
public static class Extensions
{
    public static Serilog.Core.Logger InitSerilog(this WebApplicationBuilder builder)
    {
        ...
        log.Information("MngDocument starting. Version {Version}", version);
        return log;
    }

    public static void InitWebApp(this WebApplicationBuilder builder, MngDocumentSettings settings)
    {
        ...
        builder.Services.AddControllers(options =>
            {
                options.Filters.Add<DocumentExceptionFilter>();
            })
            .AddJsonOptions(o =>
            {
    ...
    public static void UseApplicationPipeline(this WebApplication app)
    {
        ...
        if (enableSwagger)
        {
            ...
            app.UseSwaggerUI(c =>
            {
                c.SwaggerEndpoint("/api-docs/v1/swagger.json", "MngDocument API v1");
                c.RoutePrefix = "swagger";
                c.DocumentTitle = "MngDocument API";
            });
        }
...
public class SwaggerConfigureOptions : IConfigureOptions<SwaggerGenOptions>
{
    ...
    public void Configure(SwaggerGenOptions options)
    {
        foreach (var description in _provider.ApiVersionDescriptions)
        {
            ...
            {
                Title = "MngDocument API",
                Version = description.ApiVersion.ToString(),
                Description = "Document Intelligence orchestration API"
            });
        }

        var serverPath = _configuration["MngDocumentSettings:OpenApiServerPath"];
