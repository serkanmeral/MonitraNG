...
/// <summary>
/// PPTX içindeki <c>{{paramKey}}</c> ifadelerini değerlerle değiştirir (Pr3).
/// </summary>
public static class PptxPlaceholderMerger
{
        ...
        IReadOnlySet<string>? preservePlaceholderKeys = null)
    {
        ...
        using (var writeArchive = new ZipArchive(output, ZipArchiveMode.Create, leaveOpen: true))
        {
            foreach (var entry in readArchive.Entries)
            {
                ...
                if (PptxPlaceholderScanner.IsScannablePart(entry.FullName))
                {
    ...
    internal static void MergeInXmlDocument(
        ...
        IReadOnlySet<string>? preservePlaceholderKeys = null)
    {
        foreach (var textNode in doc.Descendants(A + "t").ToList())
        {
