...
**Amaç:** MngGateway (API Gateway) entegrasyonu sonrası MngHub servisinin gateway üzerinden kapsamlı test edilmesi.

**Test Kategorileri:**

1. **Gateway Routing Testleri**
   - [ ] SignalR hub erişimi (`/hub/ws/*` gateway üzerinden)
   - [ ] Health check endpoint (`/hub/health` gateway üzerinden)
   - [ ] WebSocket upgrade (gateway üzerinden)
   - [ ] Route mapping doğruluğu (gateway → MngHub)

2. **Gateway Authentication Testleri**
   - [ ] JWT token forwarding (WebSocket query string veya header)
   - [ ] Token validation (downstream service'te)
   - [ ] Unauthorized connection handling
   - [ ] Token expiration handling
   - [ ] Multi-domain token testleri

3. **SignalR Connection Testleri**
   - [ ] Gateway üzerinden SignalR bağlantısı kurma
   - [ ] Domain-based room subscription (`domain.{domainName}`)
   - [ ] Global room subscription (`global`)
   - [ ] Connection lifecycle yönetimi
   - [ ] Reconnection handling

4. **Event Broadcasting Testleri**
   - [ ] System event'lerin gateway üzerinden broadcast edilmesi
   - [ ] Domain-specific event'lerin gateway üzerinden broadcast edilmesi
   - [ ] Event routing doğruluğu (RabbitMQ → SignalR → Client)
   - [ ] Event filtering (domain isolation)
   - [ ] Multiple client connection testleri

5. **Integration Testleri**
   - [ ] End-to-end senaryolar (RabbitMQ → MngHub → Gateway → Client)
   - [ ] Multi-service integration (MngKeeper → RabbitMQ → MngHub → Gateway)
   - [ ] DataGateway event pipeline (DataGateway → RabbitMQ → MngHub → Gateway → Client)
   - [ ] Error propagation

6. **Performance Testleri**
   - [ ] Gateway overhead ölçümü (WebSocket latency)
   - [ ] Concurrent connection handling (gateway üzerinden)
   - [ ] Message throughput (events/second)
   - [ ] Connection count limits
   - [ ] Memory ve CPU kullanımı

7. **Error Handling Testleri**
...
8. **WebSocket Specific Testleri**
   - [ ] WebSocket protocol upgrade (gateway üzerinden)
   - [ ] Message framing
...
**Test Senaryoları:**
- Senaryo 1: Gateway üzerinden SignalR bağlantısı kurma
- Senaryo 2: Gateway üzerinden domain room'una subscription
- Senaryo 3: Gateway üzerinden system event alma
- Senaryo 4: Gateway üzerinden domain-specific event alma
- Senaryo 5: Gateway üzerinden multiple client connection
- Senaryo 6: Gateway üzerinden reconnection handling
- Senaryo 7: End-to-end event pipeline testi (DataGateway → Gateway → Client)
- Senaryo 8: Gateway üzerinden connection lifecycle testi

**Test Araçları:**
- PowerShell test scriptleri (gateway WebSocket URL'leri ile)
- HTML/JavaScript test sayfası (gateway WebSocket endpoint'i ile)
- Automated test suite (kurgulanacak)

**Test Ortamı:**
- Gateway URL: `wss://localhost:5040/hub/ws?access_token={token}`
- Direkt URL: `ws://localhost:5234/ws?access_token={token}` (karşılaştırma için)
- Health Check: `https://localhost:5040/hub/health`
