...
⚠️ **UI commit/push + `mngui` Odak deploy BEKLİYOR** (saf UI; backend zaten canlı, lokal dev Odak'a proxy → lokalde test edilebilir).
...
[✓] DASH-INLINE: pano board ekranında inline (ayrı sayfa yok) + Özet/Pano toggle — backend canlı, UI deploy bekliyor (2 Haz)
[~] DASH-CARDS: özet/liste kart UI zenginleştirme + "bana atanan açık işler" (currentUser→MngPersonId + wi_assigned_open) — backend CANLI (oc_live=200), UI deploy bekliyor (2 Haz)
```
