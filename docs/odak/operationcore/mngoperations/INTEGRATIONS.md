...
> **Toplu endpoint (by-ids) — UYGULANDI (BL-KB, 31 May).** Keeper'da `POST api/User/by-ids` ve `POST api/Group/by-ids` toplu uçları eklendi. User by-ids id'leri **`__dataId` veya Keycloak `sub`** olarak çözer (tek Mongo `$or/$in` sorgusu). MO `IKeeperDirectoryClient.GetUsersAsync/GetGroupsAsync` ile eksik id'leri **tek istekte** çözer (eski id-başına çağrı/N+1 giderildi); MO'nun kısa-TTL in-memory cache'i ikinci savunma hattı olarak kalır. *(Deploy: hem `mngkeeper` hem `mngoperations` gerekir.)*
>
> **Keeper Redis profil cache'i — UYGULANDI (BL-KB, 31 May).** Keeper'da `IDirectoryCache`/`DirectoryCacheService` (mevcut `IRedisService` üstünde, fail-open) eklendi: by-ids **önce Redis'ten** okur (anahtar `oc:dir:user|group:{domain}:{id}`, TTL 10dk; kullanıcı hem `__dataId` hem Keycloak `sub` anahtarıyla yazılır), eksikleri Mongo'dan çekip cache'ler. **Invalidation:** UpdateUser/DeleteUser → kullanıcı (her iki kimlik); UpdateGroup/DeleteGroup → grup. Redis erişilemezse sessizce Mongo'ya düşülür. MO'nun Keeper'ın iç Redis keyspace'ini okuması hâlâ **yapılmaz** (bu cache MO için ayrı, tutarlı keyspace). API sözleşmesi korundu, Redis hızı kazanıldı. *(Not: Keeper `GetUsers` liste ucu hâlâ Mongo'dan okur; bu cache yalnızca by-ids dizin çözümü içindir.)*
