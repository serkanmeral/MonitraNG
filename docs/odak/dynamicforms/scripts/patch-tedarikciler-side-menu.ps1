# Tedarikciler Automated Form — @side_menu kaydi (Odak DG uzerinden)
#
# Usage (repo kokunden):
...
$tedarikcilerOrder = $headerOrder + 1
...
# --- Tedarikciler CRUD formu ---
Upsert-MenuItem -AllItems $items -Label "Tedarikciler" -FindExisting {
    $_.pageCode -eq "dynamicForms.tedarikciler.menuTitle" -or
    $_.to -eq "/apps/automated-forms/view/tedarikciler-form"
} -Body @{
    order       = $tedarikcilerOrder
    itemType    = "item"
    level       = 1
    parentId    = $headerId
    pageType    = "user"
    pageCode    = "dynamicForms.tedarikciler.menuTitle"
    title       = "Tedarikciler"
    icon        = "TruckDeliveryIcon"
    iconType    = "tabler"
    to          = "/apps/automated-forms/view/tedarikciler-form"
    type        = "internal"
    disabled    = $false
    permissions = [ordered]@{
        groups = [ordered]@{
...
Write-Host "Route: /apps/automated-forms/view/tedarikciler-form" -ForegroundColor Gray
