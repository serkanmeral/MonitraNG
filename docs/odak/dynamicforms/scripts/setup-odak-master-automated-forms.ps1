# Odak Uretim master datasetleri — Automated Forms (musteri, urun grubu, urun)
#
# Usage (repo kokunden):
#   .\docs\odak\operationcore\scripts\get-operationcore-token.ps1
#   .\docs\odak\dynamicforms\scripts\setup-odak-master-automated-forms.ps1
#
# Runtime:
#   /apps/automated-forms/view/odak-musteriler-form
#   /apps/automated-forms/view/odak-urun-gruplari-form
#   /apps/automated-forms/view/odak-urunler-form

param(
    [string]$BaseUrl = "http://192.168.20.20:5040",
    [switch]$UseGateway = $true,
    [switch]$SkipSchema = $true,
    [string[]]$Forms = @(
        "odak_musteriler_automated_form.json",
        "odak_urun_gruplari_automated_form.json",
        "odak_urunler_automated_form.json"
    )
)
...
$formDir = Join-Path $repoRoot "docs/odak/dynamicforms/datasets"
$masterDatasetDir = Join-Path $repoRoot "docs/odak/is_surecleri/datasets"
$categoryFile = Join-Path $masterDatasetDir "odak_business_dataset_category.json"
...
Write-Host "Odak master — Automated Forms" -ForegroundColor Cyan
Write-Host "Gateway: $BaseUrl" -ForegroundColor Cyan
Write-Host "========================================`n" -ForegroundColor Cyan

$datasetNames = @()
foreach ($formFile in $Forms) {
    $path = Join-Path $formDir $formFile
    if (-not (Test-Path $path)) { throw "Form dosyasi yok: $path" }
    $def = Get-Content $path -Raw -Encoding UTF8 | ConvertFrom-Json
    if ($def.datasetName -and ($datasetNames -notcontains $def.datasetName)) {
        $datasetNames += $def.datasetName
    }
}

if (-not $SkipSchema) {
    Write-Host "[1] Dataset semalari..." -ForegroundColor Yellow
    $categoryId = Ensure-DatasetCategory -CategoryName "BusinessDatasets"
    foreach ($dsName in $datasetNames) {
        $dsFile = Join-Path $masterDatasetDir "${dsName}_dataset.json"
        if (-not (Test-Path $dsFile)) { throw "Dataset dosyasi yok: $dsFile" }
        Ensure-DatasetSchema -CategoryId $categoryId -DatasetFile $dsFile
    }
}
else {
...
foreach ($formFile in $Forms) {
    $path = Join-Path $formDir $formFile
    Write-Host "  -> $formFile" -ForegroundColor Cyan
    Ensure-AutomatedForm -FormFilePath $path
}

Write-Host ""
Write-Host "Tamamlandi." -ForegroundColor Cyan
Write-Host "Musteriler:    /apps/automated-forms/view/odak-musteriler-form" -ForegroundColor Gray
Write-Host "Urun gruplari: /apps/automated-forms/view/odak-urun-gruplari-form" -ForegroundColor Gray
Write-Host "Urunler:       /apps/automated-forms/view/odak-urunler-form" -ForegroundColor Gray
Write-Host "Menü:          .\docs\odak\dynamicforms\scripts\patch-odak-master-side-menu.ps1" -ForegroundColor Gray
