# Test ortami: COC-STANDARD generationProfile + primaryContextType guncelle ve publish et.
#
# Kullanim:
#   .\docs\odak\document_intelligence\scripts\patch-coc-standard-profile-test.ps1
#   .\docs\odak\document_intelligence\scripts\patch-coc-standard-profile-test.ps1 -WhatIf
#   .\docs\odak\document_intelligence\scripts\patch-coc-standard-profile-test.ps1 -SkipPublish
...
Write-Host "CoC profil patch -> $BaseUrl ($TemplateCode)" -ForegroundColor Cyan
Write-Host "Seed: $seedFile (exists=$(Test-Path $seedFile))" -ForegroundColor DarkGray
$categoryPath = @($seed.categoryPath | ForEach-Object { [string]$_ })
Write-Host "Category path: $($categoryPath -join ' > ')" -ForegroundColor DarkGray
