# Test: LINE-ACTIVITY-STD docNo -> poDocNo ayrimi (govde vs antet)
#
...
#
# Kullanim:
#   .\patch-line-activity-po-docno-test.ps1
#   .\patch-line-activity-po-docno-test.ps1 -WhatIf
#   .\patch-line-activity-po-docno-test.ps1 -SkipPublish
...
Write-Host "Activity poDocNo patch -> $BaseUrl ($TemplateCode)" -ForegroundColor Cyan
$tree = Invoke-Json -Method GET -RequestUri "$categoriesBase/tree"
$roots = if ($tree -is [System.Array]) { @($tree) } else { @($tree) }
$cat = Get-AllCategories -Nodes $roots | Where-Object { $_.path -like '*Activity*' -or $_.path -like '*Yasam*' } | Select-Object -First 1
if (-not $cat) { throw "Activity kategorisi bulunamadi" }
...
Write-Host "Dogrulama: status=$($verify.status) poDocNo=$($poParam.key)" -ForegroundColor Cyan
