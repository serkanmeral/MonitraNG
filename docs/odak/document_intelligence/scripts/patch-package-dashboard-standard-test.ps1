# Test ortami: PACKAGE-DASHBOARD-STD publish
#
# Kullanim:
#   .\docs\odak\document_intelligence\scripts\patch-shipment-list-standard-test.ps1

param(
    [string]$BaseUrl = "http://192.168.20.20:5040",
    [string]$TemplateCode = "PACKAGE-DASHBOARD-STD",
...
Write-Host "Package Dashboard publish patch -> $BaseUrl ($TemplateCode)" -ForegroundColor Cyan
...
if ($detail.status -eq "published") {
    Write-Host "Sablon zaten published." -ForegroundColor Yellow
    if ($SkipPublish) { exit 0 }
}
