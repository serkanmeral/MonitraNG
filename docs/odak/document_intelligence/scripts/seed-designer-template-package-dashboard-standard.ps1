# İş paketi kontrol paneli (XLSX) standart şablon seed
#
# Kullanım:
#   .\docs\odak\document_intelligence\scripts\build-package-dashboard-seed-xlsx.ps1
#   .\docs\odak\document_intelligence\scripts\seed-designer-template-package-dashboard-standard.ps1
...
$seedFile = Join-Path $repoRoot "docs/odak/document_intelligence/datasets/seed-designer-template-package-dashboard-standard.json"
...
if (-not (Test-Path $xlsxPath)) {
    $buildScript = Join-Path $scriptDir "build-package-dashboard-seed-xlsx.ps1"
...
Write-Host "Package Dashboard XLSX seed -> $BaseUrl" -ForegroundColor Cyan
...
try {
    ...
    Write-Host "WARN metadata PUT atlandi (from-reference zaten code tasiyor olabilir)" -ForegroundColor Yellow
}
...
Write-Host "Seed tamam. Template id: $templateId - publish icin patch-package-dashboard-standard-test.ps1 calistirin." -ForegroundColor Cyan
