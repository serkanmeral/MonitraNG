# CoC standart şablon seed — ODK-COC-template-seed.docx + parametreler + footer
#
# Kullanım:
#   .\docs\odak\document_intelligence\scripts\build-coc-seed-docx.ps1
#   .\docs\odak\document_intelligence\scripts\seed-designer-template-coc-standard.ps1
#   .\docs\odak\document_intelligence\scripts\seed-designer-template-coc-standard.ps1 -BaseUrl "http://192.168.20.8:5040"
#   .\docs\odak\document_intelligence\scripts\seed-designer-template-coc-standard.ps1 -WhatIf
...
if (-not (Test-Path $docxPath)) { throw "DOCX yok: $docxPath (once build-coc-seed-docx.ps1 calistirin)" }
...
Write-Host "CoC seed -> $BaseUrl" -ForegroundColor Cyan
...
# metadata code (from-reference may not set code)
...
Write-Host "OK letterhead (3 sutun antet: $($tpl.name))" -ForegroundColor Green

Invoke-Json -Method PUT -Uri "$templatesBase/$templateId/footer" -Body @{
    footer = $tpl.footer
} | Out-Null
Write-Host "OK footer (kurumsal altbilgi enjekte)" -ForegroundColor Green

Write-Host "Seed tamam. Template id: $templateId" -ForegroundColor Cyan
