...
#
# Siralama: odak_musteriler (legacyFirmId) -> odak_is_paketleri -> odak_siparis_kalemleri
#
# Usage (repo kokunden):
...
if (-not $SkipMaster) {
    Write-Host "[1/3] Master datasets (odak_musteriler + legacyFirmId)..." -ForegroundColor Yellow
    & (Join-Path $repoRoot "docs/odak/is_surecleri/scripts/setup-odak-master-datasets.ps1") @commonArgs
}
else {
    Write-Host "[1/3] Master atlandi (-SkipMaster)" -ForegroundColor Gray
}

Write-Host "[2/3] odak_is_paketleri..." -ForegroundColor Yellow
& (Join-Path $scriptDir "setup-odak-is-paketleri-dataset.ps1") @commonArgs

Write-Host "[3/3] odak_siparis_kalemleri (parentPackageId)..." -ForegroundColor Yellow
