# Odak Siparis — odak_sevkiyatlar + odak_sevkiyat_kalemleri dataset semalari (AF yok — hub native dialog)
#
# Usage (repo kokunden):
#   .\docs\odak\operationcore\scripts\get-operationcore-token.ps1
#   .\docs\odak\siparis\scripts\setup-odak-siparis-sevkiyat-datasets.ps1
...
Write-Host "`n=== setup-odak-siparis-sevkiyat-datasets ===" -ForegroundColor Cyan
$categoryId = Ensure-DatasetCategory -CategoryName "BusinessDatasets"
Ensure-DatasetSchema -CategoryId $categoryId -DatasetFilePath (Join-Path $datasetDir "odak_sevkiyatlar_dataset.json")
Ensure-DatasetSchema -CategoryId $categoryId -DatasetFilePath (Join-Path $datasetDir "odak_sevkiyat_kalemleri_dataset.json")
Write-Host "`nTamamlandi. Hub: expand -> Sevkiyatlar sekmesi." -ForegroundColor Cyan
