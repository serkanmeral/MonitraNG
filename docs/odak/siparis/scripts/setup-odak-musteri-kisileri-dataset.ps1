# Odak Siparis — odak_musteri_kisileri dataset + odak_is_paketleri customerContactId alani
#
# Usage (repo kokunden):
#   .\docs\odak\operationcore\scripts\get-operationcore-token.ps1
#   .\docs\odak\siparis\scripts\setup-odak-musteri-kisileri-dataset.ps1
...
Write-Host "`n=== setup-odak-musteri-kisileri-dataset ===" -ForegroundColor Cyan
$categoryId = Ensure-DatasetCategory -CategoryName "BusinessDatasets"
Ensure-DatasetSchema -CategoryId $categoryId -DatasetFilePath (Join-Path $datasetDir "odak_musteri_kisileri_dataset.json")
Ensure-DatasetSchema -CategoryId $categoryId -DatasetFilePath (Join-Path $datasetDir "odak_is_paketleri_dataset.json")

Write-Host "`nTamamlandi. Hub: Musteriler -> expand -> Kisiler." -ForegroundColor Cyan
