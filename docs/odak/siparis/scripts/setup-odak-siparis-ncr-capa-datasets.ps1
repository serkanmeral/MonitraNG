# Odak Siparis — odak_ncr + odak_capa dataset semalari (AF yok — hub native dialog)
#
# Usage (repo kokunden):
#   .\docs\odak\operationcore\scripts\get-operationcore-token.ps1
#   .\docs\odak\siparis\scripts\setup-odak-siparis-ncr-capa-datasets.ps1
...
Write-Host "`n=== setup-odak-siparis-ncr-capa-datasets ===" -ForegroundColor Cyan
$categoryId = Ensure-DatasetCategory -CategoryName "BusinessDatasets"
Ensure-DatasetSchema -CategoryId $categoryId -DatasetFilePath (Join-Path $datasetDir "odak_ncr_dataset.json")
Ensure-DatasetSchema -CategoryId $categoryId -DatasetFilePath (Join-Path $datasetDir "odak_capa_dataset.json")
Write-Host "`nTamamlandi. Hub: expand -> Kalite sekmesi (native dialog)." -ForegroundColor Cyan
