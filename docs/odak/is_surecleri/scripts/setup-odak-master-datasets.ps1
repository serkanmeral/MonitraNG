# Odak Uretim — master dataset kurulumu (musteri, urun grubu, urun)
#
# Usage (repo kokunden):
...
$datasetDir = Join-Path $repoRoot "docs/odak/is_surecleri/datasets"
$categoryFile = Join-Path $datasetDir "odak_business_dataset_category.json"
...
Write-Host "Odak Uretim — master dataset kurulumu" -ForegroundColor Cyan
...
foreach ($file in @(
        "odak_musteriler_dataset.json",
        "odak_urun_gruplari_dataset.json",
        "odak_urunler_dataset.json"
    )) {
...
Write-Host "`nTamamlandi. Sonraki: seed-odak-master-data.ps1" -ForegroundColor Cyan
