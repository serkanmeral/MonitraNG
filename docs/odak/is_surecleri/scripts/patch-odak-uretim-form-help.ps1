# Odak Uretim — Yeni emir formu layout.helpMarkdown senkronu
#
# Usage (repo kokunden):
#   .\docs\odak\operationcore\scripts\get-operationcore-token.ps1
#   .\docs\odak\is_surecleri\scripts\patch-odak-uretim-form-help.ps1
#   .\docs\odak\is_surecleri\scripts\patch-odak-uretim-form-help.ps1 -ReloadMetadataCache
...
if ([string]::IsNullOrWhiteSpace($HelpFile)) {
    $HelpFile = Join-Path $scriptDir "../seed/odak_uretim_yeni_emir_form_help.md"
}
...
$formName = "Odak Uretim - Yeni emir"
...
function ConvertTo-LayoutHashtable {
    param($LayoutObject)
    if (-not $LayoutObject) { return @{ sections = @() } }
    $raw = if ($LayoutObject -is [string]) { $LayoutObject | ConvertFrom-Json } else { $LayoutObject }
    $sections = @()
    foreach ($sec in @($raw.sections)) {
        $fields = @()
        foreach ($f in @($sec.fields)) { $fields += [string]$f }
        $sections += @{
            key    = [string]$sec.key
    ...
    $ht = @{ sections = $sections }
    foreach ($prop in @("formHeading", "formIntro", "dialogMaxWidth", "fieldCols", "sectionCols")) {
        if ($null -ne $raw.$prop) { $ht[$prop] = $raw.$prop }
    }
    return $ht
}

$layout = ConvertTo-LayoutHashtable -LayoutObject $form.layout
...
Write-Host "Tamam. Uretim panosu > Yeni is modalinda Yardim butonunu kontrol edin." -ForegroundColor Cyan
