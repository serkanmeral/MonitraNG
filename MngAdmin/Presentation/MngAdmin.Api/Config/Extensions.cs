...
using static MngAdmin.Application.Configuration.MngAdminSettings;

namespace MngAdmin.Api.Config;

public static class Extensions
{
    public static Serilog.Core.Logger InitSerilog(this WebApplicationBuilder builder, MngAdminSettings mngAdminSettings)
    {
        ...
        log.Information($"MngAdmin Starting. Version {Assembly.GetEntryAssembly()?.GetCustomAttribute<AssemblyInformationalVersionAttribute>()?.InformationalVersion.Split('+')[0]} ");
    ...
    public static void InitWebAPP(this WebApplicationBuilder builder)
    {
        ...
        var serverSettings = builder.Configuration.GetSection("MngAdminSettings:Server").Get<ServerSettings>() 
            ?? new ServerSettings();
    ...
    public static void InitAuthentication(this WebApplicationBuilder builder, MngAdminSettings settings)
    {
    ...
    public static void UseApplicationSettings(this WebApplication app, MngAdminSettings mngAdminSettings)
    {
        ...
        app.UseSwaggerUI(c =>
        {
            ...
            foreach (var description in apiVersionDescriptionProvider.ApiVersionDescriptions.OrderByDescending(d => d.ApiVersion))
            {
                c.SwaggerEndpoint(
                    ...
                    $"MngAdmin API {description.GroupName.ToUpperInvariant()}");
            }
            
            c.RoutePrefix = "swagger";
            c.DocumentTitle = "MngAdmin API Documentation";
        ...
        {
            options
                .WithTitle("MngAdmin API")
                .WithTheme(ScalarTheme.Purple)
                .WithDefaultHttpClient(ScalarTarget.CSharp, ScalarClient.HttpClient)
                .WithOpenApiRoutePattern("/api-docs/{documentName}/swagger.json");

            options.AddServer(new ScalarServer(mngAdminSettings.OpenApiServerPath, "MngAdmin Server"));
        });
