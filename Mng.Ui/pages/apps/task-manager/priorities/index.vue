...
import type { TmPriority } from '@/types/apps/taskManager';
import {
...
const deleteTarget = ref<TmPriority | null>(null);
...
const breadcrumbs = computed(() => [
  ...
  { text: mt('taskManager.priorities.title', 'Öncelikler'), disabled: true, href: '#' },
]);
...
const sortedPriorities = computed(() =>
  [...store.priorities].sort((a, b) => a.name.localeCompare(b.name, undefined, { sensitivity: 'base' }))
);

function priorityIconEl(name: string | null | undefined) {
...
function openEdit(p: TmPriority) {
  editId.value = p.__dataId;
...
function openDelete(p: TmPriority) {
...
onMounted(async () => {
  ...
  try {
    ...
    errorLocal.value = e?.message ?? mt('taskManager.priorities.loadError', 'Öncelikler yüklenemedi.');
  } finally {
    loading.value = false;
  }
});
...
async function submitForm() {
  ...
  try {
    if (editId.value) {
      await store.updatePriority(editId.value, {
    ...
    errorLocal.value = e?.message ?? mt('taskManager.priorities.saveError', 'Kaydedilemedi.');
  } finally {
    saving.value = false;
  }
}
...
async function confirmDelete() {
  ...
  errorLocal.value = null;
  try {
    await store.deletePriority(deleteTarget.value.__dataId);
    deleteDialog.value = false;
    deleteTarget.value = null;
  } catch (e: any) {
    errorLocal.value =
      e?.message ?? mt('taskManager.priorities.deleteError', 'Silinemedi (görevlerde kullanılıyor olabilir).');
    deleteDialog.value = false;
  } finally {
...
<template>
  <div class="tm-flow">
    <BaseBreadcrumb :title="mt('taskManager.priorities.title', 'Öncelikler')" :breadcrumbs="breadcrumbs" />
    ...
    <div class="tm-hero d-flex flex-column flex-md-row flex-wrap align-start align-md-center justify-space-between gap-4 mb-6">
      <div>
        <h1 class="tm-hero-title text-h4">{{ mt('taskManager.priorities.title', 'Öncelikler') }}</h1>
        <p class="tm-hero-sub mb-0">
          {{ mt('taskManager.priorities.subtitle', 'Highest, High, Medium… ortak tanımlar; görev ve filtrelerde kullanılır. Sıra alanı yok; isteğe bağlı açıklama ile anlamı netleştirin.') }}
        </p>
      </div>
      <div class="d-flex flex-wrap gap-2">
        ...
        <v-btn variant="tonal" size="large" rounded="lg" class="text-none" to="/apps/task-manager">
          ...
          {{ mt('taskManager.priorities.backToTasks', 'Görevlere dön') }}
        </v-btn>
        <v-btn color="primary" size="large" rounded="lg" class="text-none" @click="openCreate">
          <v-icon icon="mdi-plus" start />
          {{ mt('taskManager.priorities.newPriority', 'Yeni öncelik') }}
        </v-btn>
      </div>
    </div>
    ...
    <v-card class="tm-panel" rounded="xl" flat>
      <v-data-table
        :headers="[
          { title: mt('taskManager.priorities.colName', 'Ad'), key: 'name', sortable: true },
          { title: mt('taskManager.priorities.colDescription', 'Açıklama'), key: 'description', sortable: false },
          { title: mt('taskManager.priorities.colIcon', 'İkon'), key: 'icon', sortable: false },
          { title: mt('taskManager.priorities.colColor', 'Renk'), key: 'color', sortable: false },
          { title: mt('taskManager.priorities.colActions', 'İşlemler'), key: 'actions', sortable: false, align: 'end' },
        ]"
        :items="sortedPriorities"
        :loading="loading"
        class="tm-priorities-table"
      >
        ...
        <template #[`item.icon`]="{ item }">
          <div v-if="item.icon" class="d-flex align-center gap-2">
            <component
              v-if="priorityIconEl(item.icon)"
              :is="priorityIconEl(item.icon)"
              :size="20"
              class="flex-shrink-0 tm-priority-icon-cell"
            />
    ...
    <v-dialog v-model="dialog" max-width="520">
      <v-card rounded="xl">
        <v-card-title class="text-h6">
          {{ editId ? mt('taskManager.priorities.editPriority', 'Önceliği düzenle') : mt('taskManager.priorities.newPriority', 'Yeni öncelik') }}
        </v-card-title>
        <v-card-text>
          ...
          <v-text-field v-model="form.name" :label="mt('taskManager.priorities.fieldName', 'Öncelik adı')" density="comfortable" required />
          <v-textarea
            v-model="form.description"
            class="mt-3"
            :label="mt('taskManager.priorities.fieldDescription', 'Açıklama')"
            :hint="mt('taskManager.priorities.fieldDescriptionHint', 'İsteğe bağlı; örn. iş kurallarına göre ne zaman kullanılacağı.')"
            rows="2"
            auto-grow
            density="comfortable"
            variant="outlined"
          ...
          <TmStatusIconPicker
            v-model="form.icon"
            class="mt-3"
            :label="mt('taskManager.priorities.fieldIcon', 'İkon')"
          ...
          <v-select
            ...
            item-value="value"
            clearable
            :label="mt('taskManager.priorities.fieldColor', 'Tema rengi')"
    ...
    <v-dialog v-model="deleteDialog" max-width="440">
      <v-card rounded="xl">
        <v-card-title>{{ mt('taskManager.priorities.deleteTitle', 'Öncelik silinsin mi?') }}</v-card-title>
        <v-card-text>
          {{ mt('taskManager.priorities.deleteBody', 'Bu önceliğe bağlı görev varsa silme başarısız olabilir.') }}
