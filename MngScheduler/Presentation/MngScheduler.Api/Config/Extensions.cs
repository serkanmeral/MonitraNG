...
using static MngScheduler.Application.Configuration.MngSchedulerSettings;

namespace MngScheduler.Api.Config;

public static class Extensions
{
    public static Serilog.Core.Logger InitSerilog(this WebApplicationBuilder builder, MngSchedulerSettings mngSchedulerSettings)
    {
        ...
        log.Information($"MngScheduler Starting. Version {Assembly.GetEntryAssembly()?.GetCustomAttribute<AssemblyInformationalVersionAttribute>()?.InformationalVersion.Split('+')[0]} ");
    ...
    public static void InitWebAPP(this WebApplicationBuilder builder)
    {
        ...
        var serverSettings = builder.Configuration.GetSection("MngSchedulerSettings:Server").Get<ServerSettings>() 
            ?? new ServerSettings();
    ...
    public static void InitAuthentication(this WebApplicationBuilder builder, MngSchedulerSettings settings)
    {
    ...
    public static void UseApplicationSettings(this WebApplication app, MngSchedulerSettings mngSchedulerSettings)
    {
        ...
        app.UseSwaggerUI(c =>
        {
            ...
            foreach (var description in apiVersionDescriptionProvider.ApiVersionDescriptions.OrderByDescending(d => d.ApiVersion))
            {
                c.SwaggerEndpoint(
                    ...
                    $"MngScheduler API {description.GroupName.ToUpperInvariant()}");
            }
            
            c.RoutePrefix = "swagger";
            c.DocumentTitle = "MngScheduler API Documentation";
        ...
        {
            options
                .WithTitle("MngScheduler API")
                .WithTheme(ScalarTheme.Purple)
                .WithDefaultHttpClient(ScalarTarget.CSharp, ScalarClient.HttpClient)
                .WithOpenApiRoutePattern("/api-docs/{documentName}/swagger.json");

            options.AddServer(new ScalarServer(mngSchedulerSettings.OpenApiServerPath, "MngScheduler Server"));
        });
