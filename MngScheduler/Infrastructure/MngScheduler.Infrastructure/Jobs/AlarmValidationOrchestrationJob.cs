...
/// <summary>
/// MngAlarm <c>alarm-validation-*</c> user job: Keeper token → MA validation/run.
/// </summary>
[DisallowConcurrentExecution]
public class AlarmValidationOrchestrationJob : IJob
{
    private readonly IAlarmValidationOrchestrationService _orchestrationService;
    ...
    private readonly ILogger<AlarmValidationOrchestrationJob> _logger;
    ...
    public AlarmValidationOrchestrationJob(
        IAlarmValidationOrchestrationService orchestrationService,
        IMngKeeperAuthClient keeperAuth,
        IUserJobRepository userJobRepository,
        IJobExecutionRepository executionRepository,
        IRabbitMqEventPublisher eventPublisher,
        ILogger<AlarmValidationOrchestrationJob> logger)
    {
    ...
    public async Task Execute(IJobExecutionContext context)
    {
        ...
        var domainName = UserJobIds.TryGetAlarmValidationDomainName(jobId);
        var stopwatch = Stopwatch.StartNew();

        _logger.LogInformation(
            "[AlarmValidation] Quartz job started jobId={JobId} domain={Domain} executionId={ExecutionId}",
            jobId,
            domainName,
            executionId);

        if (string.IsNullOrWhiteSpace(domainName))
            throw new Quartz.JobExecutionException($"Invalid alarm validation job id: {jobId}");
        ...
        try
        {
            ...
            {
                _logger.LogWarning("[AlarmValidation] Job {JobId} should not execute, skipping", jobId);
                return;
            }

            var result = await _orchestrationService.RunValidationAsync(domainName, context.CancellationToken);
            stopwatch.Stop();
            ...
            if (string.IsNullOrWhiteSpace(responseBody) && !string.IsNullOrWhiteSpace(result.ErrorMessage))
            {
                responseBody = JsonSerializer.Serialize(new
                {
                    ...
                    result.ScheduledRaised,
                    result.ScheduledResolved
                }, JsonOptions);
            }
            ...
            {
                _logger.LogWarning("[AlarmValidation] Job {JobId} deleted during execution", jobId);
            }
            ...
            {
                _logger.LogWarning(ex, "[AlarmValidation] Event publish failed jobId={JobId}", jobId);
            }

            if (!result.IsSuccess)
                throw new Quartz.JobExecutionException($"Alarm validation failed: {result.ErrorMessage}");
        }
        catch (Exception ex) when (ex is not Quartz.JobExecutionException)
        {
            stopwatch.Stop();
            _logger.LogError(ex, "[AlarmValidation] Job failed jobId={JobId}", jobId);
            ...
            {
                _logger.LogError(saveEx, "[AlarmValidation] Failed to save execution jobId={JobId}", jobId);
            }
