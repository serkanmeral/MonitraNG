# SIEM — linux.auth.v1 → U1 alarm.raised → Workflow Event Trigger E2E
param(
...
Write-Host "=== SIEM Linux auth U1 alarm.raised -> Workflow E2E ===" -ForegroundColor Cyan

$srcIp = "10.88.$((Get-Random -Maximum 250)).$((Get-Random -Maximum 250))"
$linuxUser = "e2e_wf_$(Get-Random -Maximum 9999)"
...
$wfKey = "siem-linux-u1-wf-$(Get-Date -Format 'HHmmss')"
$def = Invoke-RestMethod -Uri "$wf/definitions" -Method POST -Headers $hdr -Body (@{
    key  = $wfKey
    name = "SIEM Linux U1 Workflow E2E"
} | ConvertTo-Json)
...
$verBody = @{
    ...
    nodes       = @(
        ...
        @{
            id     = "log_1"
            type   = "write.log"
            config = @{ message = "SIEM Linux U1 alarm user={{event.context.userId}} srcIp={{event.context.srcIp}} severity={{event.severity}}" }
        }
    )
...
$ruleName = "U1 Linux WF E2E $(Get-Date -Format 'HHmmss')"
    ...
    $body = @{
        items = @(
            @{
                ...
                source     = @{ type = "endpoint"; product = "linux-syslog"; host = "bastion-linux-u1-wf" }
                raw        = $linuxLine
            }
        )
...
Write-Host "   run=$($detail.instance.id) log_1 OK srcIp=$srcIp" -ForegroundColor Green
Write-Host "`nOK SIEM linux.auth.v1 -> alarm.raised -> workflow PASS" -ForegroundColor Green
exit 0
