# SIEM U8 — Windows AD group_member_added (4728) sec_events → observation → correlation alarm
param(
...
$ruleFixture = Join-Path $fixtureDir "alarm_rules/u8_ad_group_member_added.json"
...
Write-Host "=== SIEM U8 AD group_member_added correlation E2E ===" -ForegroundColor Cyan

Write-Host "`n1) U8 correlation rule (threshold=1, cooldown=0)..." -ForegroundColor Cyan
$ruleTemplate = Get-Content -Path $ruleFixture -Raw | ConvertFrom-Json
$ruleName = "U8 SIEM E2E $(Get-Date -Format 'HHmmss')"
...
$rawObj = Read-FixtureJson "windows_4728_group_member_added.json"
$adminUser = "u8_admin_$(Get-Random -Maximum 9999)"
$rawObj.SubjectUserName = $adminUser
$receivedAt = (Get-Date).ToUniversalTime().ToString("yyyy-MM-ddTHH:mm:ss.fffZ")

Write-Host "`n2) POST 1 x group_member_added sec-event (user=$adminUser)..." -ForegroundColor Yellow
$body = @{
    items = @(
        @{
...
Write-Host "`n3) Alarm raised bekleniyor (severity>=9)..." -ForegroundColor Cyan
$found = $false
for ($i = 0; $i -lt 20; $i++) {
    Start-Sleep -Seconds 2
    $page = Invoke-RestMethod -Uri "$alarm/alarms?openOnly=true&minSeverity=9" -Headers $hdr
    $items = @($page.items)
...
if (-not $found) {
    if ($FailIfSkipped) { throw "U8 alarm raised bulunamadi" }
    Write-Host "   UYARI: Alarm raised bulunamadi (FailIfSkipped yok)" -ForegroundColor Yellow
    exit 0
}

Write-Host "`nOK SIEM U8 group_member_added -> alarm PASS" -ForegroundColor Green
exit 0
