# SIEM — linux.auth.v1 → U1 alarm → approval → block.ip (Reactor mqtt/publish)
param(
...
Write-Host "=== SIEM Linux auth U1 approval -> block.ip E2E ===" -ForegroundColor Cyan
$resolvedEngineId = Get-EngineId
$srcIp = "10.88.$((Get-Random -Maximum 250)).$((Get-Random -Maximum 250))"
$linuxUser = "e2e_blk_$(Get-Random -Maximum 9999)"
Write-Host "   engineId=$resolvedEngineId srcIp=$srcIp user=$linuxUser" -ForegroundColor DarkGray

Write-Host "`n1) Workflow (alarm.raised -> log -> approval -> block.ip -> log)..." -ForegroundColor Cyan
$wfKey = "siem-linux-u1-block-$(Get-Date -Format 'HHmmss')"
$def = Invoke-RestMethod -Uri "$wf/definitions" -Method POST -Headers $hdr -Body (@{
    key  = $wfKey
    name = "SIEM Linux U1 Approval Block E2E"
} | ConvertTo-Json)
...
$verBody = @{
    ...
    nodes       = @(
        ...
        @{
            id     = "log_ctx_1"
            type   = "write.log"
            config = @{ message = "SIEM Linux U1 ctx srcIp={{event.context.srcIp}} user={{event.context.userId}}" }
        },
        ...
        @{
            ...
            config = @{
                ...
                ttlMinutes = 60
                reason     = "SIEM Linux U1 brute-force block"
            }
        },
        @{ id = "log_ok_1"; type = "write.log"; config = @{ message = "SIEM Linux U1 block.ip completed" } },
        @{ id = "log_reject_1"; type = "write.log"; config = @{ message = "SIEM Linux U1 rejected" } }
    )
    edges = @(
...
$rule = Invoke-RestMethod -Uri "$alarm/rules" -Method POST -Headers $hdr -Body (@{
    name             = "U1 Linux Block E2E $(Get-Date -Format 'HHmmss')"
    type             = "correlation"
...
Write-Host "`n3) POST $Threshold x linux.auth.v1 login_failed sec-events..." -ForegroundColor Yellow
for ($i = 0; $i -lt $Threshold; $i++) {
    $body = @{
        items = @(
            @{
                receivedAt = $receivedAt
                source     = @{ type = "endpoint"; product = "linux-syslog"; host = "bastion-linux-u1-block" }
                raw        = $linuxLine
            }
        )
...
Invoke-RestMethod -Uri "$wf/approvals/$approvalId/decide" -Method POST -Headers $hdr -Body (@{
    ...
    comment   = "SIEM Linux U1 E2E approve block"
...
Write-Host "   run=$instanceId mode=reactor_mqtt ip=$srcIp" -ForegroundColor Green
Write-Host "`nOK SIEM linux.auth.v1 -> approval -> block.ip PASS" -ForegroundColor Green
exit 0
