# Smoke: F3-1 mimari is paketi (Odak test)
param(
...
Write-Host "F3-1 smoke  code=$code  gateway=$Gateway" -ForegroundColor Cyan

try {
    $packs = @(Invoke-Ops -Path "/job-packs")[0]
    $packList = @($packs)
    $codes = @($packList | ForEach-Object { $_.code })
    Assert-True ($codes -contains "pmo") "katalog pmo duruyor"
    Assert-True ($codes -contains "quality") "katalog quality duruyor"
    Assert-True ($codes -contains "architecture") "katalog architecture"

    $arch = $packList | Where-Object { $_.code -eq "architecture" } | Select-Object -First 1
    $expected = Count-Wbs $arch.wbs
    Assert-True ($expected -ge 8) "mimari wbs onizleme=$expected"
    Assert-True (@($arch.kinds) -contains "adr") "tur adr"
    Assert-True (@($arch.kinds) -contains "icd") "tur icd"
    Assert-True (@($arch.folders) -contains "C4") "klasor C4"
    Assert-True (@($arch.folders) -contains "ADR") "klasor ADR"

    $empty = @(Invoke-Ops -Method POST -Path "/projects" -Body @{
            code   = "$code-E"
            name   = "F3-1 empty $stamp"
            status = "active"
        } -ExpectStatus @(201, 200))[0]
    $emptyId = [string]$empty.id
    $preview = @(Invoke-Ops -Path "/projects/$emptyId/packs/architecture/preview?intent=apply")[0]
    ...
    $created = @(Invoke-Ops -Method POST -Path "/projects" -Body @{
            code     = $code
            name     = "F3-1 smoke $stamp"
            status   = "active"
            packCode = "architecture"
        } -ExpectStatus @(201, 200))[0]
    ...
    Assert-True ($names -contains "C4 bağlam" -or ($names -join ",") -match "C4") "C4 WBS"
    Assert-True ($names -contains "ADR") "ADR WBS"
    Assert-True ($names -contains "ICD") "ICD WBS"
    Assert-True ($names -contains "Mimari baseline") "mimari baseline"
    $ms = @($detail.wbs | Where-Object { $_.kind -eq "milestone" })
    Assert-True ($ms.Count -ge 1) "kilometre tasi=$($ms.Count)"

    $reapply = @(Invoke-Ops -Method POST -Path "/projects/$projectId/packs/architecture?mode=skip")[0]
    ...
    Write-Host "F3-1 smoke OK" -ForegroundColor Green
}
catch {
    Write-Host "F3-1 smoke FAIL: $($_.Exception.Message)" -ForegroundColor Red
