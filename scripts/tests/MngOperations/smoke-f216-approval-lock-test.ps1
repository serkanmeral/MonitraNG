# Smoke: F2-16 onaysiz belge kapatma kilidi (Odak test)
param(
...
Write-Host "F2-16 smoke  code=$code  gateway=$Gateway" -ForegroundColor Cyan

try {
    $created = @(Invoke-Ops -Method POST -Path "/projects" -Body @{
            code     = $code
            name     = "F2-16 approval lock $stamp"
            status   = "active"
            packCode = "pmo"
    ...
    $draft = @(Invoke-Doc -Method POST -Path "/markdown" -Body @{
            parentId = $hubId
            title    = "Taslak kanit"
            content  = "draft"
            isDraft  = $true
    ...
    $pub = @(Invoke-Doc -Method POST -Path "/markdown" -Body @{
            parentId = $hubId
            title    = "Yayin kanit"
            content  = "published"
            isDraft  = $false
    ...
    Invoke-Ops -Method POST -Path "/wbs/$kickId/evidence" -Body @{ resourceId = $draftId } | Out-Null
    Invoke-Ops -Method POST -Path "/wbs/$kickId/references" -Body @{ resourceId = $pubId } | Out-Null

    $status = @(Invoke-Ops -Path "/projects/$projectId/status")[0]
    $kickStatus = @(Get-StatusItems $status | Where-Object { $_.wbsId -eq $kickId -or $_.WbsId -eq $kickId } | Select-Object -First 1)[0]
    Assert-True (Test-Flag $kickStatus "missingApproval") "status missingApproval"
    ...
    $blocked = @(Invoke-Ops -Method POST -Path "/work-items/$kickWi/transitions/resolve" -Body @{
            fields = @{ description = "approval lock smoke" }
        } -ExpectStatus @(409))[0]
    Assert-True ((Get-Code $blocked) -eq "APPROVAL_REQUIRED") "resolve APPROVAL_REQUIRED"

    Invoke-Doc -Method POST -Path "/$draftId/lifecycle" -Body @{ action = "approve" } | Out-Null
    Assert-True ($true) "taslak yayinlandi"

    Invoke-Ops -Method POST -Path "/work-items/$kickWi/transitions/resolve" -Body @{
            fields = @{ description = "approval lock smoke" }
        } | Out-Null
    Assert-True ($true) "yayin sonrasi resolve serbest"
    ...
    Write-Host "F2-16 smoke PASSED" -ForegroundColor Green
}
catch {
    Write-Host "F2-16 smoke FAILED: $_" -ForegroundColor Red
    if (-not $KeepArtifacts) {
        foreach ($id in $projectIds) {
