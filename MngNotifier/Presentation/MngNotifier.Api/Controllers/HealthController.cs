...
namespace MngNotifier.Api.Controllers
{
    ...
    public class HealthController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetHealth()
        {
            try
            {
                ...
                _logger.LogInformation(
                    "Health check completed: {Status} (RabbitMQ: {RabbitStatus}, Disk: {DiskStatus})",
                    result.Status,
        ...
        public async Task<IActionResult> Ready()
        {
            ...
            // For readiness, we need RabbitMQ to be healthy
            // RabbitMQ is optional for now (not yet implemented)
            var isReady = result.Checks.RabbitMQ.Status == "healthy" || result.Checks.RabbitMQ.Status == "degraded";
