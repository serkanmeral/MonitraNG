...
7. **API Gateway Sonrası Kapsamlı Test Süreci** - Gateway entegrasyonu testleri (Planlandı)
...
## 🔬 API Gateway Sonrası Kapsamlı Test Süreci

**Amaç:** MngGateway (API Gateway) entegrasyonu sonrası MngKeeper servisinin gateway üzerinden kapsamlı test edilmesi.

**Test Kategorileri:**

1. **Gateway Routing Testleri**
   - [ ] Tüm endpoint'lerin gateway üzerinden erişilebilirliği (`/keeper/api/*`)
...
2. **Gateway Authentication Testleri**
   ...
   - [ ] Token generation (`/keeper/api/auth/token`)
...
3. **Endpoint Functional Testleri**
   - [ ] Domain creation (`/keeper/api/domain` - POST)
   - [ ] Domain list (`/keeper/api/domain` - GET)
   - [ ] Domain details (`/keeper/api/domain/{domainName}` - GET)
   - [ ] User CRUD işlemleri (`/keeper/api/user/*`)
   - [ ] Group CRUD işlemleri (`/keeper/api/group/*`)
   - [ ] User-Group assignment operations
   - [ ] Password management (change, reset)

4. **Integration Testleri**
   - [ ] End-to-end senaryolar (gateway → MngKeeper → Keycloak/MongoDB/Redis)
   - [ ] Domain creation pipeline (12 adım gateway üzerinden)
   - [ ] Multi-service integration (MngKeeper → DataGateway sync)
   - [ ] Event publishing (RabbitMQ events)
...
8. **Domain Creation Pipeline Testleri**
   - [ ] Gateway üzerinden domain oluşturma (tüm 12 adım)
   - [ ] Pipeline step failures handling
   - [ ] Rollback mechanism testleri
   - [ ] Domain activation testleri

**Test Senaryoları:**
- Senaryo 1: Gateway üzerinden token alma
- Senaryo 2: Gateway üzerinden domain oluşturma (end-to-end pipeline)
- Senaryo 3: Gateway üzerinden user CRUD işlemleri
- Senaryo 4: Gateway üzerinden group CRUD işlemleri
- Senaryo 5: Gateway üzerinden user-group assignment
- Senaryo 6: Gateway üzerinden password management
- Senaryo 7: Gateway üzerinden domain list ve details
- Senaryo 8: Gateway üzerinden error handling senaryoları
...
- Direkt URL: `https://localhost:5001/api/*` (karşılaştırma için)
